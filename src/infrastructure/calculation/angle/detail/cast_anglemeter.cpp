...
void anglemeterSetImageSize(anglemeter_t* am, const int width, const int height) {
    if (am->img_width == width && am->img_height == height) return;
    am->img_width = width;
...
inline void scanCol(anglemeter_t* am, const rgb_t* img, const int x, const int y_from, const int y_to)
{
    ...
    const int bright_lim = am->config.bright_lim;
    const int max_pairs  = am->config.max_pairs;
    ...
    for (int y = y_from; y < y_to; ++y) {
        const rgb_t& p1 = img[(static_cast<size_t>(y) - am->config.scan_step * 2) * width + x];
        ...
        const int yEdge = y - am->config.scan_step;
...
inline void scanRow(anglemeter_t* am, const rgb_t* img, const int y, const int x_from, const int x_to)
{
    ...
    for (int x = x_from; x < x_to; ++x) {
        const rgb_t& p1 = img[y * width + (static_cast<size_t>(x) - am->config.scan_step * 2)];
        const rgb_t& p2 = img[y * width + static_cast<size_t>(x)];

        const int br1 = brightness(p1.r, p1.g, p1.b);
        const int br2 = brightness(p2.r, p2.g, p2.b);
        const int dBr = br2 - br1;
        const int xEdge = x - am->config.scan_step;
...
inline bool scanColsDirectional(
    ...
    *count = 0;
    ...
    int last_y_from = std::max(am->config.scan_step * 2, static_cast<int>(scans[x_start].posDifMin.y) - 8);
    int last_y_to   = std::min(height - am->config.scan_step * 2, static_cast<int>(scans[x_start].posDifMax.y) + 8);
    ...
    for (int x = x_start + x_step;
         ...
         x += x_step)
    {
        const int y_from = std::max(am->config.scan_step * 2, last_y_from - 8);
        const int y_to   = std::min(height - am->config.scan_step * 2, last_y_to + 8);
        ...
        last_y_from = std::clamp(static_cast<int>(scans[x].posDifMin.y), am->config.scan_step * 2, height - am->config.scan_step * 2);
        last_y_to   = std::clamp(static_cast<int>(scans[x].posDifMax.y), am->config.scan_step * 2, height - am->config.scan_step * 2);
...
inline bool scanCols(anglemeter_t* am, const rgb_t* img, int* dir)
{
    ...
    const int margin = am->config.scan_step * 2;
...
inline bool estimateBothAngles(const float angle_1, const float angle_2, float* out_angle) {
    ...
    const float diff = std::fabs(angle_1 - angle_2);
