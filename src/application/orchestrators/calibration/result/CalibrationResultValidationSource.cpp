                    ...
                    for (const auto& point : result.points()) {
                        validation.addIssue(
                            ...
                            CalibrationResultValidationIssue{
                                ...
                                CalibrationValidationIssueKind::SpanTooLarge,
                                "Угол шкалы больше 280°"
                            });
                    }
                }
                ...
                if (span < min_span) {
                    for (const auto& point : result.points()) {
                        validation.addIssue(
                            ...
                            CalibrationResultValidationIssue{
                                CalibrationIssueSeverity::Error,
                                CalibrationValidationIssueKind::SpanTooSmall,
                                "Угол шкалы меньше 260°"
                            });
                    }
                }
                ...
                for (const auto& point : result.points()) {
                    ...
                    if (std::abs(forward_angle - backward_angle) > hysteresis_limit) {
                        const auto issue = CalibrationResultValidationIssue{
                            ...
                            CalibrationValidationIssueKind::HysteresisTooLarge,
