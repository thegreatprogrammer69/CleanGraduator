...
void QtInfoSettingsWidget::buildUi() {
    ...
    fileLoggingEnabledCheckBox_ = new QCheckBox(this);
    ...
    maxCenterDeviationSpinBox_->setSingleStep(0.1);
    maxCenterDeviationSpinBox_->setSuffix(QStringLiteral(" °"));
    ...
    addRow(tr("Логирование в файл"), fileLoggingEnabledCheckBox_);
...
void QtInfoSettingsWidget::connectUi() {
    ...
    connect(fileLoggingEnabledCheckBox_, &QCheckBox::stateChanged, this, [this](int state) {
        model_.fileLoggingEnabled.set(state == Qt::Checked);
        model_.save();
    });

    connect(maxCenterDeviationSpinBox_, qOverload<double>(&QDoubleSpinBox::valueChanged), this, [this](double value) {
        model_.maxCenterDeviationDeg.set(static_cast<float>(value));
        model_.save();
    });
}
...
void QtInfoSettingsWidget::connectViewModel() {
    ...
    fileLoggingEnabledSub_ = model_.fileLoggingEnabled.subscribe([this](const auto& ev) {
        if (fileLoggingEnabledCheckBox_->isChecked() != ev.new_value) {
            fileLoggingEnabledCheckBox_->setChecked(ev.new_value);
        }
    }, false);
    ...
    maxCenterDeviationSub_ = model_.maxCenterDeviationDeg.subscribe([this](const auto& ev) {
        ...
        if (!qFuzzyCompare(maxCenterDeviationSpinBox_->value() + 1.0, value + 1.0)) {
            maxCenterDeviationSpinBox_->setValue(value);
        }
    }, false);
    ...
    fileLoggingEnabledCheckBox_->setChecked(model_.fileLoggingEnabled.get_copy());
